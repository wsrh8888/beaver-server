...
type FriendByFriendshipId struct {
...
type FriendVerifySyncReq struct {
	UserID      string `header:"Beaver-User-Id"`
	FromVersion int64  `json:"fromVersion"`    // 起始版本号
	ToVersion   int64  `json:"toVersion"`      // 结束版本号
	Limit       int    `json:"limit,optional"` // 限制数量，默认100
...
type FriendVerifySyncRes struct {
...
type GetFriendsListByFriendshipIdsReq struct {
	FriendshipIds []string `json:"friendshipIds"` // 好友关系ID列表
}

type GetFriendsListByFriendshipIdsRes struct {
	Friends []FriendByFriendshipId `json:"friends"` // 好友列表
}
