...
type CreateMomentReq struct {
	...
	Visibility int64            `json:"visibility,optional"` // 可见性：0=所有人（默认）1=仅好友 2=仅自己
	AllowList  []string         `json:"allowList,optional"`  // 白名单用户ID（可见性=3时指定可见人）
	BlockList  []string         `json:"blockList,optional"`  // 黑名单用户ID（不让谁看）
}
