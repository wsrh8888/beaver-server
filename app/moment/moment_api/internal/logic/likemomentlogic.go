...
func (l *LikeMomentLogic) LikeMoment(req *types.LikeMomentReq) (resp *types.LikeMomentRes, err error) {
	...
	if req.Status {
		...
		// 检查是否已点赞
		result := l.svcCtx.DB.Where("moment_id = ? AND like_user_id = ?", req.MomentID, req.UserID).First(&like)
		...
		// 取消点赞操作
		if err := l.svcCtx.DB.Where("moment_id = ? AND like_user_id = ?", req.MomentID, req.UserID).Delete(&moment_models.MomentLikeModel{}).Error; err != nil {
			return nil, err
		}
	}
