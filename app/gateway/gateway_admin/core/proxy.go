...
func (p Proxy) auth(res http.ResponseWriter, req *http.Request) (ok bool) {
	...
	// 执行鉴权接口
	authAddr := etcd.GetServiceAddr(p.Config.Etcd, "auth_admin")
	if authAddr == "" {
		return false
	}

	authUrl := fmt.Sprintf("http://%s/admin/auth/authentication", authAddr)
...
func (p Proxy) ServeHTTP(res http.ResponseWriter, req *http.Request) {
	...
	service := addrList[1]
	addr := etcd.GetServiceAddr(p.Config.Etcd, service+"_admin")
	if addr == "" {
		logx.Error("未匹配到服务")
