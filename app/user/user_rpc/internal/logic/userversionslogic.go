...
func NewUserVersionsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UserVersionsLogic {
	return &UserVersionsLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
		logger: beaverlog.New("user_versions", ctx),
	}
}
...
func (l *UserVersionsLogic) UserVersions(in *user_rpc.UserVersionsReq) (*user_rpc.UserVersionsRes, error) {
	...
	if err != nil {
		l.logger.Error(model.LogMsg{
			Text: "查询用户版本信息失败",
			Data: map[string]any{"userIds": in.UserIds, "err": err.Error()},
		})
		return nil, err
	}
	...
	for _, userId := range in.UserIds {
		if _, exists := resp.UserVersions[userId]; !exists {
			resp.UserVersions[userId] = 0 // 用户不存在返回版本号
		}
	}

	l.logger.Info(model.LogMsg{
		Text: "查询用户版本信息完成",
		Data: map[string]interface{}{"resultCount": len(userList), "requestCount": len(in.UserIds)},
	})
	return resp, nil
}
