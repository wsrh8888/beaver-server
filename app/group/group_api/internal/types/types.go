...
type GroupJoinRequestSyncRes struct {
	GroupJoinRequests []GroupJoinRequestSyncItem `json:"groupJoinRequests"` // 群组申请列表
...
type GroupMemberSyncRes struct {
	GroupMembers []GroupMemberSyncItem `json:"groupMembers"` // 群成员列表
	HasMore      bool                  `json:"hasMore"`      // 是否还有更多数据
	NextVersion  int64                 `json:"nextVersion"`  // 下次同步的起始版本号
}
...
type GroupSyncRes struct {
	Groups      []GroupSyncItem `json:"groups"`      // 群组列表
	HasMore     bool            `json:"hasMore"`     // 是否还有更多数据
	NextVersion int64           `json:"nextVersion"` // 下次同步的起始版本号
}
