...
func NewListVersionsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *ListVersionsLogic {
	return &ListVersionsLogic{ctx: ctx, svcCtx: svcCtx, logger: beaverlog.New("list_versions", ctx)}
}
...
func (l *ListVersionsLogic) ListVersions(in *platform_rpc.ListVersionsReq) (*platform_rpc.ListVersionsRes, error) {
	...
	if err := db.Count(&total).Error; err != nil {
		l.logger.Error(model.LogMsg{Text: "统计版本失败", Data: map[string]any{"architectureId": in.ArchitectureId, "err": err.Error()}})
		return nil, err
	}
	...
	if err := db.Offset((page - 1) * pageSize).Limit(pageSize).Order("created_at DESC").Find(&list).Error; err != nil {
		l.logger.Error(model.LogMsg{Text: "查询版本列表失败", Data: map[string]any{"architectureId": in.ArchitectureId, "err": err.Error()}})
		return nil, err
	}
