...
func (l *DeleteMomentLogic) DeleteMoment(req *types.DeleteMomentReq) (resp *types.DeleteMomentRes, err error) {
	...
	// 可选：同时删除相关的评论和点赞
	// 删除评论
	err = l.svcCtx.DB.Where("moment_id = ?", req.Id).Delete(&moment_models.MomentCommentModel{}).Error
	if err != nil {
		logx.Errorf("删除动态评论失败: %v", err)
	}

	// 删除点赞
	err = l.svcCtx.DB.Where("moment_id = ?", req.Id).Delete(&moment_models.MomentLikeModel{}).Error
	if err != nil {
