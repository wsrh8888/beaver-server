...
type WsReq struct {
	...
	Token    string `form:"token"`    // 用户token（JWT）
	Platform string `form:"platform"` // 平台类型：windows/macos/linux/ios/android（后端通过 User-Agent 二次校验）
	DeviceID string `form:"deviceId"` // 设备唯一标识 (GUID)，用于设备指纹校验
}
